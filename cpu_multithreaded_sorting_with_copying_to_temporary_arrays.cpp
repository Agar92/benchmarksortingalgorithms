...
public:
  ParallelSorter(const std::vector<P>& input, size_t num_threads = std::thread::hardware_concurrency())
    ...
    void sort_and_count() {
        ...
        for (size_t i = 0; i < num_threads_; ++i) {
            threads_.emplace_back([this, i]() {
                ...
                data.sorted_subarray.resize(size);
                std::copy(input_.begin() + data.start_idx,
                          input_.begin() + data.end_idx,
                          data.sorted_subarray.begin());

                std::sort(data.sorted_subarray.begin(), data.sorted_subarray.end(), [](P & l, P & r){return l.ir<r.ir;});

                // 2. Count occurrences of 0,1,2,3
                for (const P & p : data.sorted_subarray) {
                    int val=p.ir;
                    if (val >= 0 && val <= 3) {
                        data.counts[val]++;
                    }
                    ///cout<<"val="<<val<<" data.counts["<<val<<"]="<<data.counts[val]<<"   ";
                }

                /*
                cout<<endl;
                cout<<"@@@data.sorted_subarray:";
                for(int i=0; i<data.sorted_subarray.end()-data.sorted_subarray.begin(); ++i)
                  cout<<data.sorted_subarray[i]<<" ";
                cout<<endl;
                cout<<"@@@data.start_idx="<<data.start_idx<<" data.end_idx="<<data.end_idx<<endl;
                cout<<"@@@CHECK data.counts[] in sort_and_count():"<<endl;
                for(int I=0; I<4; ++I)
                  cout<<data.counts[I]<<"   ";
                cout<<endl;
                */
    ...
    void copy_blocks() {
        ...
        for (size_t i = 0; i < num_threads_; ++i) {
            threads_.emplace_back([this, i]() {
                auto& data = thread_data_[i];
                const auto& sorted = data.sorted_subarray;
                /*
                cout<<"***sorted: ";
                ...
                */
                ...
                for (int val = 0; val <= 3; ++val) {
                    ...
                    if (count > 0) {
                        ...
                        /*
                        cout<<"*** CHECK IN val="<<val<<" count="<<count<<" dest_pos="<<dest_pos
                            <<" data.write_offsets["<<val<<"]="<<data.write_offsets[val]<<":   ";
                        for(int I=0; I<count; ++I)
                        {
                          cout<<sorted[i+read_pos]<<" ";
                        }
                        cout<<endl;
                        */
                        // Copy the block of this value
                        std::memcpy(output_.data() + dest_pos,
                                    sorted.data() + read_pos,
                                    count * sizeof(P));
                        read_pos += count;
                    }
                }
...
public:
    OpenMPParallelSorter(const std::vector<P>& input)
    ...
    std::vector<P> sort() {
        ...
        std::vector<std::vector<P>> sorted_subarrays(num_threads);
        ...
        ////cout<<"### tid="<<tid<<endl;
            ...
            // Extract and sort subarray
            sorted_subarrays[tid].resize(end_idx - start_idx);
            std::copy(input_.begin() + start_idx,
                     input_.begin() + end_idx,
                     sorted_subarrays[tid].begin());

            std::sort(sorted_subarrays[tid].begin(), sorted_subarrays[tid].end(), [](P & l, P & r){return l.ir<r.ir;});

            // Count values
            for (const P & p : sorted_subarrays[tid]) {
                int val=p.ir;
                if (val >= 0 && val <= 3) {
                    counts[tid][val]++;
                }
        ...
        */
        ...
        {
            int tid = omp_get_thread_num();
            const auto& sorted = sorted_subarrays[tid];
...
int main() {
    ...
    for (size_t size : test_sizes) {
        ...
        */
        ...
        for(int i=0; i<seq_result.size(); ++i) cout<<seq_result[i]<<" ";
        cout<<endl;
        */
        ...
        for(int i=0; i<par_result.size(); ++i) cout<<par_result[i]<<" ";
        cout<<endl;
        */
    ...
    ////#ifdef _OPENMP
        ...
        for(int i=0; i<omp_result.size(); ++i) cout<<omp_result[i]<<" ";
        cout<<endl;
        */
